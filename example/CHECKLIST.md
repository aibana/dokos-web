...
| Item | Estado | Notas |
|------|--------|-------|
| Doctype HTML5 | ✅ Listo | `BaseLayout.astro:13` |
| Charset UTF-8 | ✅ Listo | `BaseLayout.astro:16` |
| Viewport | ✅ Listo | `BaseLayout.astro:17` — incluye `viewport-fit=cover` |
| Title | ✅ Listo | vía `astro-seo` en `Head.astro` |
| Description | ✅ Listo | vía `astro-seo` en `Head.astro` |
| Favicon SVG + ICO | ✅ Listo | `/public/favicon.svg` y `/public/favicon.ico` |
| Apple Web App meta | ❌ Falta | No hay `apple-touch-icon.png` ni meta tags de Apple |
| Windows Tiles | ❌ Falta | No hay `browserconfig.xml` |
| Canonical URL | ✅ Listo | `Head.astro` vía `astro-seo` |

### ✅ HEAD — HTML Tags

| Item | Estado | Notas |
|------|--------|-------|
| Lang attribute | ✅ Listo | `<html lang={locale}>` en `BaseLayout.astro:14` |
| Direction attribute | ⚠️ Parcial | Falta `dir="ltr"` explícito (todos los idiomas actuales son LTR) |
| Alternate hreflang | ✅ Listo | 16 idiomas en `Head.astro:50-52` |
| x-default hreflang | ✅ Listo | `Head.astro:53` |
| RSS feed | N/A | No aplica (no es un blog) |
| CSS Critical inline | ❌ Falta | Sin CSS crítico inlineado |
| CSS order (antes de JS) | ✅ Listo | Umami tiene `defer`, no bloquea render |

### ✅ HEAD — Social Meta

| Item | Estado | Notas |
|------|--------|-------|
| Facebook OG: title, type, image, url | ✅ Listo | `Head.astro` vía `astro-seo` |
| OG: description, locale, siteName | ✅ Listo | `Head.astro` |
| OG: image width/height (1200×630) | ✅ Listo | `Head.astro:41-46` — agregado |
| Twitter Card | ❌ Omitido | Sin cuenta de Twitter — omisión intencional |

### ✅ HTML — Best Practices

| Item | Estado | Notas |
|------|--------|-------|
| HTML5 Semantic Elements | ✅ Listo | `<main>`, `<header>`, `<footer>`, `<nav>`, `<section>` |
| Error 404 | ✅ Listo | `src/pages/404.astro` con CSS integrado |
| Error 5xx | ❌ Falta | No hay página de error de servidor |
| Noopener en links externos | ⚠️ Verificar | Revisar links con `target="_blank"` |
| Comentarios innecesarios | ⚠️ Verificar | Hay algunos comentarios de debug para revisar |

### ✅ HTML — Testing

| Item | Estado | Notas |
|------|--------|-------|
| W3C Validator | ❌ Pendiente | Ejecutar en https://validator.w3.org/ |
| HTML Lint | ❌ Pendiente | Ejecutar webhint u otro linter |
| Link checker (sin 404) | ⚠️ Bloqueante | URLs de descarga en `releases.json` están vacías |
| Adblockers test | ❌ Pendiente | Verificar que Umami no rompe la UI al estar bloqueado |

### ✅ Webfonts

| Item | Estado | Notas |
|------|--------|-------|
| Formato WOFF/WOFF2 | ✅ Listo | Google Fonts sirve WOFF2 automáticamente |
| Tamaño < 2MB | ✅ Listo | Inter + JetBrains Mono + Material Symbols vía CDN |
| font-display: swap | ✅ Listo | `?display=swap` en todas las URLs de Google Fonts |
| Webfont loader | ✅ Listo | Defer con `onload` + `<noscript>` fallback |

### ✅ CSS

| Item | Estado | Notas |
|------|--------|-------|
| Responsive Web Design | ✅ Listo | Tailwind CSS mobile-first |
| CSS Print | ❌ Falta | Sin `@media print` stylesheet |
| Unique ID | ✅ Listo | IDs únicos por página |
| Reset CSS | ✅ Listo | Tailwind preflight incluye normalize |
| Embedded/inline CSS | ⚠️ Parcial | Hay `<style>` blocks en componentes (válido en Astro) |
| Vendor prefixes | ✅ Listo | Tailwind/PostCSS autoprefixer |
| CSS Minification | ✅ Listo | Astro minifica en build |
| Non-blocking CSS | ✅ Listo | JetBrains Mono y Material Symbols son diferidos |
| W3C CSS Validator | ❌ Pendiente | Ejecutar en https://jigsaw.w3.org/css-validator/ |
| Responsive breakpoints test | ❌ Pendiente | Probar en 320px, 768px, 1024px |
| Cross-browser test | ❌ Pendiente | Chrome, Firefox, Safari, Edge |
| Cross-OS test | ❌ Pendiente | Windows, macOS, iOS, Android |
| RTL support | ❌ Falta | No hay soporte para idiomas RTL |

### ✅ Images

| Item | Estado | Notas |
|------|--------|-------|
| Optimización | ⚠️ Verificar | Imágenes son PNG estáticos, sin pipeline de optimización |
| Picture/srcset | ❌ Falta | Sin `<picture>` ni `srcset` para responsive images |
| Retina (2x/3x) | ❌ Falta | Sin variantes de alta densidad |
| Alt text | ✅ Listo | Todos los `<img>` tienen `alt` descriptivo |
| Width/Height en `<img>` | ✅ Listo | Agregado en todos los componentes |
| Lazy loading | ✅ Listo | `loading="lazy"` en imágenes below the fold; `eager` en above-the-fold |
| WebP format | ❌ Falta | Imágenes en PNG, sin conversión a WebP |

### ✅ JavaScript

| Item | Estado | Notas |
|------|--------|-------|
| Sin JS inline mezclado con HTML | ✅ Listo | Scripts en bloques `<script>` separados (Astro pattern) |
| Concatenación/Minificación | ✅ Listo | Astro bundlea y minifica en build |
| noscript tag | ✅ Listo | `<noscript>` para fonts diferidas en `BaseLayout.astro` |
| Non-blocking JS | ✅ Listo | Scripts con `defer` o `is:inline` |
| ESLint | ❌ Pendiente | No hay ESLint configurado en el proyecto |

### ✅ Security

| Item | Estado | Notas |
|------|--------|-------|
| HTTPS | ✅ Listo | Site configurado en `https://dokos.app` |
| HSTS header | ❌ Falta | Configurar en servidor/CDN |
| CSRF | N/A | Sitio estático, sin formularios con mutación |
| XSS | ✅ Listo | Astro escapa outputs por defecto; sin `innerHTML` directo |
| X-Content-Type-Options | ❌ Falta | Header del servidor: `nosniff` |
| X-Frame-Options | ❌ Falta | Header del servidor: `SAMEORIGIN` |
| Content Security Policy | ❌ Falta | Sin CSP definido |

### ✅ Performance

| Item | Estado | Notas |
|------|--------|-------|
| First Meaningful Paint < 1s | ❌ Pendiente | Ejecutar Lighthouse para medir |
| Google PageSpeed ≥ 90 | ❌ Pendiente | Ejecutar en producción |
| HTML minificado | ✅ Listo | Astro minifica en build por defecto |
| Lazy loading | ✅ Listo | Imágenes y fonts diferidas |
| DNS prefetch | ⚠️ Parcial | Hay `preconnect` (incluye DNS); falta `dns-prefetch` como fallback |
| Preconnect | ✅ Listo | `BaseLayout.astro:33-38` (Google Fonts, Umami) |
| Preload | ✅ Listo | Inter preloaded como font crítica |

### ✅ Accessibility

| Item | Estado | Notas |
|------|--------|-------|
| Progressive enhancement | ⚠️ Parcial | El modal y nav funcionan solo con JS habilitado |
| Color contrast WCAG AA | ❌ Pendiente | Verificar en https://www.siegemedia.com/contrast-ratio |
| H1 en todas las páginas | ✅ Listo | Homepage y 404 tienen H1 |
| Headings en orden correcto | ⚠️ Verificar | Revisar jerarquía H1→H2→H3 en index.astro |
| Skip to content | ✅ Listo | `BaseLayout.astro:86-91` |
| Labels en formularios | N/A | Sin formularios en el sitio |
| WAVE accessibility test | ❌ Pendiente | Ejecutar en http://wave.webaim.org/ |
| Keyboard navigation | ✅ Listo | Modal (Escape), Navbar, language switcher con teclado |
| Screen-reader test | ❌ Pendiente | Probar con VoiceOver o NVDA |
| Focus style visible | ✅ Listo | Clases `focus:` de Tailwind en elementos interactivos |

### ✅ SEO

| Item | Estado | Notas |
|------|--------|-------|
| Google Analytics | N/A | Se usa Umami (privacy-friendly) en lugar de GA |
| Google Search Console | ❌ Pendiente | Registrar dominio y enviar sitemap |
| Headings logic | ⚠️ Verificar | Revisar que los headings describan el contenido para SEO |
| sitemap.xml | ✅ Listo | Generado por `@astrojs/sitemap`, referenciado en robots.txt |
| robots.txt | ✅ Listo | `/public/robots.txt` — no bloqueante |
| Structured Data (JSON-LD) | ✅ Listo | `SoftwareApplication` + `Organization` en `Head.astro` |
| Rich Results test | ❌ Pendiente | Validar en https://search.google.com/test/rich-results |
| HTML Sitemap | ❌ Falta | Sin sitemap HTML visible para usuarios en el footer |

---

### Resumen ejecutivo

| Prioridad | Total | Listo | Pendiente |
|-----------|-------|-------|-----------|
| 🔴 Alta | 18 | 12 | 6 |
| 🟡 Media | 22 | 10 | 12 |
| 🟢 Baja | 12 | 3 | 9 |
...
#### 🔴 Pendientes de alta prioridad (no omitibles)

1. **URLs de descarga vacías** — `src/config/releases.json` tiene todos los campos `""` — el modal no descarga nada
2. **W3C Validator** — ejecutar en https://validator.w3.org/
3. **Link checker** — ejecutar W3C Link Checker para detectar enlaces rotos
4. **Google PageSpeed ≥ 90** — ejecutar Lighthouse en producción
5. **WAVE accessibility test** — ejecutar en http://wave.webaim.org/
6. **Keyboard navigation** — testear todo el sitio navegando solo con teclado

#### 🟡 Pendientes de media prioridad (antes de lanzar)

- Headers de seguridad en servidor: **HSTS**, **X-Frame-Options**, **X-Content-Type-Options**
- **Content Security Policy** (CSP)
- **Noopener** en links externos con `target="_blank"` — verificar
- **Color contrast** WCAG AA — verificar con herramienta
- **CSS Print** stylesheet (`@media print`)
- **Responsive breakpoints** test (320px, 768px, 1024px)
- **Cross-browser** test (Chrome, Firefox, Safari, Edge)
- **Cross-OS** test (Windows, macOS, iOS, Android)
- **WebP** — convertir imágenes PNG a WebP para mejor performance
- **Picture/srcset** — imágenes responsive según viewport
- **Screen-reader** test (VoiceOver / NVDA)
- **Google Search Console** — registrar dominio

#### 🟢 Pendientes de baja prioridad (post-lanzamiento)

- `apple-touch-icon.png` — para dispositivos iOS
- `browserconfig.xml` + Windows Tiles — para Windows
- `favicon.png` — formato PNG adicional al SVG
- `dir="ltr"` explícito en `<html>`
- **HTML Sitemap** en el footer
- **DNS prefetch** como fallback al preconnect
- **Retina images** (2x/3x)
- **ESLint** — configurar para el proyecto
- **Error 5xx** page
